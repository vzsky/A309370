...
struct Elems {
  ...
  void print (int n = N, std::ostream& os = std::cout) const {
    os << "size: " << size() << "\n";
    for (const auto& x : *this) { x.print(n, os); os << '\n'; }
  }
  ...
  bool contains (const Elem<N>& x) const {
    for (const auto & e: mVec) if (e == x) return true; 
    return false;
  }
...
template<int N>
Elems<N> parse_from_file(const std::string& filename) {
