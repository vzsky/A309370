...
private:
  ...
  struct node {
    ...
    size_type pri, sz;
    node_ptr l, r;
    node(value_type v) { val = v; pri = rand(); sz = 1; }
  };
  ...
  inline bool value_eq (const value_type &a, const value_type &b) const { return value_le(a, b) && value_ge(a, b); }

  inline size_type sz(const node_ptr &t) const { return t?t->sz:0; }
  inline void upd(node_ptr &t) const { if (t) t->sz = sz(t->l) + sz(t->r) + 1; }

  template<typename Func>
  void inorder(Func f, const node_ptr& t) const {
    if (!t) return;
    inorder(f, t->l);
    f(t->val);
    inorder(f, t->r);
  ...
  void split(node_ptr &l, node_ptr &r, node_ptr &&t, value_type val) const { // <= val in l
    if (!t) {
      l = nullptr;
      r = nullptr;
      return;
  ...
  void splitat(node_ptr &l, node_ptr &r, node_ptr &&t, size_type pos) const { // first pos elements at l
    if (!t) {
      l = nullptr;
      r = nullptr;
      return;
  ...
  void insert (node_ptr &t, node_ptr && n) const {
    ...
    else if (value_le(t->val, n->val)) insert(t->r, std::move(n));
    else insert(t->l, std::move(n));
    upd(t);
  }
